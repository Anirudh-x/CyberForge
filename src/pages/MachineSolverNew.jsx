...
// --- LOGIC HELPERS (FULL RESTORATION) ---
...
const MachineSolver = () => {
  ...
  const handleFlagSubmit = async (e, vulnerabilityInstanceId) => {
    ...
    try {
      ...
      if (data.correct) {
        ...
        // --- FIX: SYNC DATA AND AUTO-NAVIGATE ---
        const [statsData, machineData, solutionsData] = await Promise.all([
          fetchUserStats(), 
          fetchMachine(), 
          fetchMachineSolutions()
        ]);

        // Find next unsolved vector
        const updatedSolvedIds = statsData?.stats?.solvedVulnerabilities?.filter(v => v.machineId === id).map(v => v.vulnerabilityInstanceId) || [];
        const nextUnsolved = machine.vulnerabilities.find(v => !updatedSolvedIds.includes(v.vulnerabilityInstanceId));
        
        if (nextUnsolved) {
            setActiveVulnerability(nextUnsolved);
        }
  ...
  const renderLabHeader = () => {
    ...
    const currentPoints = solvedVulns.reduce((acc, curr) => acc + (curr.points || 0), 0);
  ...
  const renderTerminalTab = () => {
    ...
    return (
      <div className="bg-[#020202] border-x border-b border-white/10 p-16 h-[750px] flex flex-col justify-center relative">
         <div className="absolute inset-0 opacity-5 pointer-events-none flex flex-wrap gap-4 text-[10px] p-4 text-emerald-900 overflow-hidden select-none">
            {Array.from({length: 100}).map((_, i) => <span key={i}>0x{Math.random().toString(16).slice(2, 8).toUpperCase()}</span>)}
        </div>
          ...
          <div key={vuln.vulnerabilityInstanceId} className={`p-10 border shadow-2xl transition-all duration-700 relative overflow-hidden group
            ...
            <div className="flex justify-between items-start mb-10">
              <div className="flex items-center gap-10">
                  {/* --- HOVER GLOW ADDED TO INDEX --- */}
                  <span className="text-6xl font-black text-emerald-950/20 italic font-mono transition-all duration-500 group-hover:text-emerald-500 group-hover:drop-shadow-[0_0_20px_rgba(16,185,129,0.8)]">
                    0{idx + 1}
                  </span>
            ...
            {!isSolved ? (
              <form onSubmit={(e) => handleFlagSubmit(e, vuln.vulnerabilityInstanceId)} className="space-y-6 relative z-10">
                ...
                {result && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className={`p-4 text-[11px] font-black uppercase tracking-[0.3em] border flex items-center gap-3 ${result.correct ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'bg-red-500/10 border-red-500 text-red-500 shadow-[0_0_20px_rgba(239,68,68,0.1)]'}`}>
  ...
  const renderReportUpload = () => {
    ...
    return (
      <footer className={`mt-10 p-10 border rounded-sm relative overflow-hidden ${!isFullySolved ? 'bg-black/40 border-white/5 opacity-50' : 'bg-emerald-500/5 border-emerald-500/30 shadow-[0_0_50px_rgba(16,185,129,0.1)]'}`}>
        {!isFullySolved ? (
          <div className="flex flex-col items-center justify-center gap-4 text-zinc-700 font-black uppercase text-[11px] tracking-[0.5em] italic">
            <Lock size={18} className="opacity-20"/> ACCESS_DENIED // CAPTURE_REQUIRED ({solvedVulns.length}/{totalVulns})
          </div>
        ) : (
          <div className="flex flex-col md:flex-row justify-between items-center gap-10 relative z-10">
            <div className="space-y-2">
              <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter glow-text flex items-center gap-3 text-emerald-500">DEBRIEF_PROTOCOL</h3>
  ...
  if (loading) return (
    <div className="h-screen bg-black flex flex-col items-center justify-center text-emerald-500 font-mono relative overflow-hidden">
        <div className="w-80 h-1 bg-white/5 mb-8 rounded-full overflow-hidden border border-white/10 relative z-10 shadow-[0_0_30px_#000]">
            <motion.div initial={{ x: '-100%' }} animate={{ x: '100%' }} transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }} className="w-1/2 h-full bg-emerald-500 shadow-[0_0_25px_#10b981]" />
        </div>
        <p className="text-[11px] font-black tracking-[1em] animate-pulse relative z-10 uppercase glow-text">Calibrating_Neural_Sync</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-black text-white font-mono selection:bg-emerald-500 selection:text-black relative">
      <div className="fixed inset-0 pointer-events-none z-[9999] opacity-[0.05] [background:linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] [background-size:100%_2px,3px_100%]"></div>
        ...
        <main className="flex-1 flex flex-col bg-black relative overflow-hidden">
                    ...
                    return (
                      <div key={idx} className={`p-12 border-2 transition-all duration-700 relative overflow-hidden group shadow-[0_20px_50px_rgba(0,0,0,0.5)]
                        ...
                        <div className="space-y-10">
                            ...
                            {/* FLAG NOW REVEALED IN REAL-TIME */}
                            {isSolved && solution.flag && (
                                <div className="p-8 bg-black border-2 border-emerald-500/20 space-y-4 shadow-[inset_0_0_30px_rgba(0,0,0,1)] relative">
                                    ...
                                    <code className="text-lg font-black text-white block break-all font-mono glow-text p-4 bg-emerald-500/5 border border-emerald-500/10 tracking-widest uppercase">{solution.flag}</code>
                                </div>
                            )}
                        </div>
                      </div>
          ...
          <section className="flex-1 flex flex-col overflow-hidden relative z-10 p-12">
            <div className="max-w-[1500px] mx-auto w-full h-full flex flex-col">
                {renderTabBar()}
                <div className="flex-1 overflow-y-auto custom-scrollbar pt-12 pr-4">
