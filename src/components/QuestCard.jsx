...
export default function QuestCard({ title, points, description, hint, id, url, alreadySolved = false }) {
  const [flag, setFlag] = useState('');
  const [message, setMessage] = useState('');
  const [status, setStatus] = useState(alreadySolved ? 'success' : '');
  ...
  return (
    <div className="bg-black text-green-400 border border-green-500 p-6 rounded-md shadow-md relative">
      ...
      <div 
        ...
        style={{ 
          maxHeight: isHintVisible ? '600px' : '0', 
          overflow: 'hidden' 
        }}
      >
